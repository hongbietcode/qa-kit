---
name: scenario
description: "Generate test cases, edge cases, and QA scenarios by decomposing a feature or code path across 12 dimensions. Use for pre-implementation risk discovery, QA planning, regression design, and iterative saturation when coverage must be exhaustive."
when_to_use: "Invoke to expand requirements, a spec, or a code path into test cases and edge cases."
argument-hint: "<file path or feature description> [--iterations N] [--saturation] [--format test-scenarios]"
license: MIT
metadata:
  kit: qa-kit
  version: "1.2.0"
...
| Automate scenarios as E2E/API/load tests | `web-testing` | Map each Critical/High row to a Playwright, API, or k6 test |
| Explore scenarios live in a browser | `agent-browser` | Walk Critical/High rows as exploratory QA charters |
| Inform implementation plan risks | the project's plan file | Paste Critical/High rows into the risk section |
...
/qa-kit:scenario src/api/payment.ts
/qa-kit:scenario "User registration with OAuth providers"

# Bounded iterative — exactly 25 iterations
/qa-kit:scenario src/api/payment.ts --iterations 25

# Saturation — stop when coverage exhausted
/qa-kit:scenario "Add multi-tenancy to the database layer" --saturation

# Saturation with domain hint for priority dimension ordering
/qa-kit:scenario src/middleware/auth.ts --saturation --domain security
```

In Codex, mention the skill as `$qa-kit:scenario` with the same arguments.
