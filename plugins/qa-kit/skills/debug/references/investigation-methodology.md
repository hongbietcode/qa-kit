...
- Use native file search (or parallel read-only Explore subagents) to find relevant files whenever context is missing or conflicting
- Use `repomix` only when a broad packaged snapshot materially improves the investigation; do not generate a mandatory summary document
- Read current package/plugin documentation from the official docs site or an installed docs-lookup tool
