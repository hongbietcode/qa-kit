...
metadata:
  kit: qa-kit
  version: "2.1.0"
  attribution: "Techniques derived from microsoft/amplifier agent patterns (MIT)"
---
...
For broken code or a failing test, use the `debug` skill instead: prove the root
cause first, then return here only if the fix design itself is stuck.
...
`../brainstorm/references/ultra-verifier-mode.md`. It is a best-of-5
