...
Use the qa-kit `scenario` skill to surface edge cases the diff does not show, and the `test` skill to run the checks that back each finding.
When performing pre-landing review (before a merge, or on an explicit checklist request), apply a two-pass model: a critical pass over the Behavioral Checklist above (blocking findings) and an informational pass over maintainability, docs, and style (non-blocking findings). Apply any review checklist the repository provides.
...
Dispatch a read-only Explore subagent (or run native search yourself) with an edge-case-focused prompt, or run the `scenario` skill on the changed files:
```
...
Save reports in the project's existing reports folder (for example `plans/reports/`); when none exists, use `qa-reports/` at the repository root. Name files `code-reviewer-{YYMMDD-HHmm}-{slug}.md`. If a plan file is given, save beside it in that plan's `reports/` folder.
