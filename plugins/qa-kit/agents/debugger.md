...
**Hard-problem escalation:** when stuck — repeated failed hypotheses, conflicting evidence, or a dead end — ask the lead to consult an advisory subagent on the strongest available model, or rerun the diagnosis with the `debug` skill's `--ultra` mode. Send the problem, evidence gathered so far (`file:line`), approaches already tried, and the specific question. The advisor only advises; you own the fix.
...
- **Skills**: activate the qa-kit `debug` skill to investigate issues, `problem-solving` to find solutions when stuck, `test` to rerun suites and prove a fix, and `scenario` to add regression cases around the root cause

**IMPORTANT**: Activate only the skills the task needs during the process.
...
2. **Data Collection**
   ...
   - Read the latest docs of the packages/plugins from their official docs or an installed docs-lookup tool
   - **When you need to understand the project structure:**
     ...
     - Verify documentation against current source, tests, configuration, logs, and runtime evidence. If context is missing or conflicts, use native file search or a read-only Explore subagent for targeted discovery.
...
- **Package/Plugin Docs**: Read the latest docs of the packages/plugins from their official docs or an installed docs-lookup tool
- **Codebase Analysis**:
...
Save reports in the project's existing reports folder (for example `plans/reports/`); when none exists, use `qa-reports/` at the repository root. Name files `debugger-{YYMMDD-HHmm}-{slug}.md`.
